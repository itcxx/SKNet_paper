...
def parse_args():
    ...
    parser.add_argument("--config", type=str, default="config/config_HandPose_TwoInput.yaml", help="Path to config file")
    ...
    # 新增工作文件夹配置参数
    parser.add_argument("--work_dir", type=str, default="./work_dir/config_HandPose_TwoInput", help="Working folder path, used to save the configuration files and logs used in this training")
...
def main():
    ...
    if dataset_name == 'Interhand99k':
        ...
        if input_views !=2:
            ...
            train_dataset = HandPoseDatasetTw(config_path=args.config, mode="train", transform=train_transform)
            val_dataset = HandPoseDatasetTw(config_path=args.config, mode="val", transform=val_transform)
    else:
    ...
    for epoch in range(start_epoch, config["train"]["epochs"]):
        ...
        with torch.no_grad():
            for batch in val_bar:
                ...
                imgs_val = batch["fusion_img"].to(device)
